...
//
// Created by Yingjie Cheng on 2022/08/08
//
////////////////////////////////////////////////////////
...
/// Return a double value in [-1 1)
double GetRandomNumber()
{
    double randValue = rand() / (RAND_MAX + 1.0);

    randValue = 2 * randValue - 1;
...
Matrix4d GetTranslateMatrix(double x, double y, double z)
{
    Matrix4d TransAffineM;
    TransAffineM.setIdentity();
    TransAffineM(0,3) = x;
...
Matrix4d GetRotationMatrix(Vector3d StartV, Vector3d EndV)
{
    StartV.normalize();
    EndV.normalize();
    Vector3d rotA;
    rotA = StartV.cross(EndV);
    Matrix4d rotM;

    if(rotA.norm() < 0.00000001)
    {
        if(StartV.dot(EndV) > 0.0)
            rotM.setIdentity();
        else
        {
            Vector3d N1 = Vector3d (1,0,0).cross(StartV);
            if(N1.norm() < 0.00000001)
                N1 = Vector3d (0,1,0).cross(StartV);
            rotM = GetRotationMatrix(M_PI, N1);
        }
    }
    else
    {
        double theta = acos(StartV.dot(EndV));
        rotM = GetRotationMatrix(theta, rotA);
    }
    return rotM;
}
...
Matrix4d InterpolationMat4d(Matrix4d matA, Matrix4d matB, double t)
{
    Matrix3d A, B;
    A = matA.block(0,0,3,3);
    B = matB.block(0, 0, 3, 3);
    Quaterniond quaterA(A);
    Quaterniond quaterB(B);
    Quaterniond quad = quaterA.slerp(t,quaterB);
    Matrix3d RotM = quad.toRotationMatrix();
    Vector3d tranM = matA.block(0,3,3,1)*(1-t) + matB.block(0,3,3,1)*t;
    return GetAffine(RotM, tranM);
}