...
int LMFunctorIK::operator()(const Eigen::VectorXd &x, Eigen::VectorXd &fvec) const
{
    ...
    //end-effector
    Vector3d realEnd;
    MultiplyPoint(effector_point, linkMats[effector_ID], realEnd);
    realEnd = (realEnd - InputEndP) / Uniscale;
    fvec(12) = realEnd.x();
    fvec(13) = realEnd.y();
    fvec(14) = realEnd.z();
...
int LMFunctorIK::df(const VectorXd &x, MatrixXd &fjac) const {
    ...
    double tol = 0.0001;
    epc.resize(n);
    vector<Matrix4d> linkMatsPlus, linkMatsMini;
    for(int itr=0;itr<n;itr++)
    {
        epc.setZero();
        epc(itr) = tol;
        Matrix4d M5plus = LoopMatrix(x+epc, linkMatsPlus);
        Matrix4d M5mini = LoopMatrix(x-epc, linkMatsMini);
        Matrix4d D_M5_i = (M5plus - M5mini)/(2.0*tol);
        Matrix4d D_M2_i = (linkMatsPlus[effector_ID] - linkMatsMini[effector_ID])/(2.0*tol);
        Vector3d DiffP;
        MultiplyPoint(effector_point, D_M2_i, DiffP);
        fjac(12, itr) = DiffP.x() / Uniscale;
        fjac(13, itr) = DiffP.y() / Uniscale;
        fjac(14, itr) = DiffP.z() / Uniscale;
...
Matrix4d LMFunctorIK::LoopMatrix(const VectorXd &x, vector<Matrix4d> &linkMats) const
{
    linkMats.clear();
